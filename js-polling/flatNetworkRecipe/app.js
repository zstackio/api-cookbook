...
var addLocalPrimaryStorage = (ipAddress, sessionUuid) => {
  requestPromise({
    ...
    .then((response) => {
      polling(response, sessionUuid, "localPrimaryStorafge", attachPrimaryStorageToCluster);
    })
}
...
var attachPrimaryStorageToCluster = (ipAddress, sessionUuid) => {
  requestPromise({
      method: 'POST',
      url: ipAddress + '/v1/clusters/' + allInventor.cluster.uuid + '/primary-storage/' + allInventor.localPrimaryStorafge.uuid,
      headers: {
        'Authorization': 'OAuth ' + sessionUuid
      },
      resolveWithFullResponse: true,
    ...
    .then((response) => {
      polling(response, sessionUuid, "attachPrimaryStorageToCluster", addSftpBackupStorage);
    })
}
