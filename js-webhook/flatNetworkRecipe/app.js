...
router.post('/add-local-primary-storage', async(ctx, next) => {
  if (ctx.request.header["x-job-success"] != "true")
    return
  allInventory.localPrimaryStorafge = ctx.request.body.inventory;
  ctx.response.status = 200;
  console.log('addLocalPrimaryStorage');
  request({
    method: 'POST',
    url: ipAddress + '/v1/clusters/' + allInventory.cluster.uuid + '/primary-storage/' + allInventory.localPrimaryStorafge.uuid,
    headers: {
      'Authorization': 'OAuth ' + sessionUuid,
...
router.post('/add-public-ip-range', async(ctx, next) => {
  ...
  request({
    ...
    headers: {
      'Authorization': 'OAuth ' + sessionUuid,
      'X-Web-Hook': 'http://172.31.253.203:8000/add-dns-to-public-l3network'
    }
  })
})

router.post('/add-dns-to-public-l3network', async(ctx, next) => {
  if (ctx.request.header["x-job-success"] != "true")
    return
